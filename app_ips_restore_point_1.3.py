...
class IPSApplication:
    ...
    def verify_invoices(self, regime):
        ...
        try:
            ...
            # Título
            ttk.Label(frame, text=f"Resultados de Verificación - {regime}", 
                     font=("Arial", 14, "bold")).grid(row=0, column=0, pady=(0, 20))
            ...
            # Crear Treeview
            columns = ("factura", "estado", "mensaje")
            ...
            tree.column("factura", width=150, anchor="center")
            tree.column("estado", width=100, anchor="center")
            tree.column("mensaje", width=800, anchor="w")
            ...
            # Verificar cada carpeta de factura
            for folder in sorted(invoice_folders):
    ...
    def correct_contract_numbers(self, regime, tree):
        ...
        try:
            ...
            for item in tree.get_children():
                ...
                # Si el mensaje indica un problema con el contrato
                if "número de contrato" in message.lower():
                    folder_path = os.path.join(facturas_path, folder)
                    ...
                    if xml_files:
                        ...
                        try:
                            ...
                            # Buscar y reemplazar el número de contrato
                            # Asumimos que el formato es "ContractNumber">XXXX.2025<
                            import re
                            pattern = r'ContractNumber">[^<]+<'
                            new_content = re.sub(pattern, f'ContractNumber">{self.contract_number}<', content)
                            ...
                            tree.set(item, "estado", "OK")
                            tree.set(item, "mensaje", "Número de contrato corregido")
                            corrected += 1
