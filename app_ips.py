...
class IPSApplication:
            ...
            if option == "Verificación de Facturas":
                ttk.Button(button_frame, text=option, 
                          ...
                          style='Regime.TButton').grid(row=row, column=col, 
                                                     padx=5, pady=5, 
                                                     sticky="ew")
            elif option == "Verificar Soportes":
                ttk.Button(button_frame, text=option, 
                          command=lambda r=regime: self.verify_supports(r),
                          style='Regime.TButton').grid(row=row, column=col, 
                                                     padx=5, pady=5, 
                                                     sticky="ew")
            elif option == "Empaquetar Facturas":
                ttk.Button(button_frame, text=option, 
                          command=lambda r=regime: self.package_invoices(r),
                          style='Regime.TButton').grid(row=row, column=col, 
                                                     padx=5, pady=5, 
                                                     sticky="ew")
            elif option == "Empaquetar soportes":
                ttk.Button(button_frame, text=option, 
                          command=lambda r=regime: self.package_supports(r),
                          style='Regime.TButton').grid(row=row, column=col, 
                                                     padx=5, pady=5, 
                                                     sticky="ew")
    ...
    def verify_supports(self, regime):
        ...
        try:
            # Obtener la ruta de la carpeta de soportes
            soportes_path = self.directories[regime]['SOPORTES']
            
            # Verificar si la carpeta existe
            if not os.path.exists(soportes_path):
                self.log_to_console(f"Error: La carpeta de soportes no existe en {soportes_path}")
                return
            
            # Obtener todos los archivos
            files = [f for f in os.listdir(soportes_path) 
                    if os.path.isfile(os.path.join(soportes_path, f))]
            
            if not files:
                self.log_to_console("No se encontraron archivos de soportes.")
                return
            
            # Crear ventana de resultados
            results_window = tk.Toplevel(self.root)
            results_window.title(f"Verificación de Soportes - {regime}")
            results_window.geometry("1200x600")
            ...
            # Título
            ttk.Label(frame, text=f"Verificación de Soportes - {regime}", 
                     font=("Arial", 14, "bold")).grid(row=0, column=0, pady=(0, 20))
            ...
            # Treeview
            tree = ttk.Treeview(tree_frame, columns=("factura", "crc", "fev", "hev", "pde", "estado"),
                               show="headings", yscrollcommand=scrollbar.set)
            
            # Configurar columnas
            tree.heading("factura", text="Número de Factura")
            tree.heading("crc", text="CRC")
            tree.heading("fev", text="FEV")
            tree.heading("hev", text="HEV")
            tree.heading("pde", text="PDE")
            tree.heading("estado", text="Estado")
            
            # Configurar anchos de columna
            tree.column("factura", width=150)
            tree.column("crc", width=100, anchor="center")
            tree.column("fev", width=100, anchor="center")
            tree.column("hev", width=100, anchor="center")
            tree.column("pde", width=100, anchor="center")
            tree.column("estado", width=600)
            ...
            # Diccionario para agrupar archivos por número de factura
            facturas = {}
            ...
            for file in files:
                ...
                if file_type not in ['CRC', 'FEV', 'HEV', 'PDE']:
                    continue
                ...
                invoice_number = parts[2].split('.')[0]  # Quitar extensión
                if not invoice_number.startswith(self.prefix):
                    continue
                
                # Agregar al diccionario de facturas
                if invoice_number not in facturas:
                    facturas[invoice_number] = {
                        'crc': '❌',
                        'fev': '❌',
                        'hev': '❌',
                        'pde': '❌',
                ...
                # Marcar tipo de archivo como encontrado
                facturas[invoice_number][file_type.lower()] = '✓'
            
            # Insertar resultados en el Treeview
            for invoice_number, status in facturas.items():
                # Verificar si faltan archivos
                missing = []
                for file_type in ['crc', 'fev', 'hev', 'pde']:
                    if status[file_type] == '❌':
                        missing.append(file_type.upper())
                
                # Actualizar estado
                if missing:
                    status['estado'] = f"Faltan los siguientes archivos: {', '.join(missing)}"
                else:
            ...
            self.log_to_console(f"Verificación de soportes completada para {regime}")
            
        except Exception as e:
            self.log_to_console(f"Error durante la verificación: {str(e)}")
            messagebox.showerror("Error", f"Error durante la verificación: {str(e)}")

    def package_supports(self, regime):
        """Empaquetar soportes en archivos ZIP, agrupando por facturas completas y limitando a 50MB por paquete"""
        try:
            soportes_path = self.directories[regime]['SOPORTES']
            empaquetado_path = self.directories[regime]['EMPAQUETADO']
            
            # Crear directorio de empaquetado si no existe
            if not os.path.exists(empaquetado_path):
                os.makedirs(empaquetado_path)
            ...
            if not files:
                self.log_to_console(f"✗ No se encontraron archivos en {soportes_path}")
                return
            
            # Crear ventana de progreso
            progress_window = tk.Toplevel(self.root)
            progress_window.title("Progreso de Empaquetado")
            ...
            # Etiqueta de estado
            status_label = ttk.Label(progress_frame, text="Iniciando empaquetado...")
            ...
            # Contador
            counter_label = ttk.Label(progress_frame, text="0/0")
            counter_label.grid(row=2, column=0, sticky=tk.W)
            
            # Actualizar ventana
            progress_window.update()
            
            # Agrupar archivos por factura
            facturas = {}
            for file in files:
                parts = file.split('_')
                if len(parts) != 3:
                ...
                invoice_number = parts[2].split('.')[0]
                if not invoice_number.startswith(self.prefix):
                    continue
                ...
                file_path = os.path.join(soportes_path, file)
                file_size = os.path.getsize(file_path)
                
                facturas[invoice_number]['files'].append(file)
                facturas[invoice_number]['size'] += file_size
                facturas[invoice_number]['types'].add(file_type)
            
            # Filtrar solo facturas completas (con los 4 tipos de archivo)
            complete_facturas = {
                inv: data for inv, data in facturas.items() 
                if len(data['types']) == 4
            }
            
            if not complete_facturas:
                self.log_to_console("✗ No se encontraron facturas con soportes completos")
                progress_window.destroy()
                return
            
            # Crear paquetes
            MAX_PACKAGE_SIZE = 50 * 1024 * 1024  # 50MB en bytes
            current_package = []
            current_size = 0
            package_number = 1
            total_packages = 0
            total_facturas = len(complete_facturas)
            processed_facturas = 0
            
            # Ordenar facturas por número
            sorted_facturas = sorted(complete_facturas.items())
            
            for invoice_number, data in sorted_facturas:
                # Si agregar esta factura excedería el límite, crear nuevo paquete
                if current_size + data['size'] > MAX_PACKAGE_SIZE and current_package:
                ...
                # Agregar factura al paquete actual
                current_package.append((invoice_number, data))
                current_size += data['size']
                processed_facturas += 1
                
                # Actualizar interfaz
                status_label.config(text=f"Procesando factura {invoice_number}...")
                progress_bar['value'] = (processed_facturas / total_facturas) * 100
                counter_label.config(text=f"{processed_facturas}/{total_facturas}")
                progress_window.update()
            
            # Crear el último paquete si hay facturas pendientes
            if current_package:
                self._create_support_package(
                    current_package, 
                    regime, 
            ...
            self.log_to_console(f"✓ Proceso de empaquetado completado: {total_packages} paquetes creados")
            
        except Exception as e:
            self.log_to_console(f"✗ Error en el proceso de empaquetado: {str(e)}")
            if 'progress_window' in locals():
                progress_window.destroy()

    def _create_support_package(self, package_data, regime, package_number, soportes_path, empaquetado_path):
        """Crear un paquete ZIP con los soportes de las facturas especificadas"""
        try:
            # Crear archivo ZIP
            zip_filename = f"SOPORTES_{regime}_PAQUETE_{package_number:03d}.zip"
            zip_path = os.path.join(empaquetado_path, zip_filename)
            
            # Crear archivo de manifiesto
            manifest_filename = f"SOPORTES_{regime}_PAQUETE_{package_number:03d}_MANIFIESTO.txt"
            manifest_path = os.path.join(empaquetado_path, manifest_filename)
            
            with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                # Agregar archivos al ZIP
                for invoice_number, data in package_data:
            ...
            with open(manifest_path, 'w', encoding='utf-8') as f:
                f.write(f"MANIFIESTO DE SOPORTES - {regime}\n")
                f.write(f"Paquete: {package_number:03d}\n")
                f.write(f"Fecha: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                f.write("\nFacturas incluidas:\n")
                for invoice_number, _ in package_data:
                    f.write(f"- {invoice_number}\n")
            
            self.log_to_console(f"✓ Paquete {package_number} creado exitosamente")
            
        except Exception as e:
            self.log_to_console(f"✗ Error al crear el paquete {package_number}: {str(e)}")
            raise

    def package_invoices(self, regime):
        """Empaquetar facturas basándose en los manifiestos de soportes"""
        try:
            facturas_path = self.directories[regime]['FACTURAS']
            empaquetado_path = self.directories[regime]['EMPAQUETADO']
            ...
            if not os.path.exists(facturas_path):
                self.log_to_console(f"✗ No se encontró el directorio de facturas: {facturas_path}")
                return
            if not os.path.exists(empaquetado_path):
                self.log_to_console(f"✗ No se encontró el directorio de empaquetado: {empaquetado_path}")
                return
            
            # Buscar archivos de manifiesto
            manifest_files = [f for f in os.listdir(empaquetado_path) 
                            if f.endswith('_MANIFIESTO.txt') and f.startswith(f'SOPORTES_{regime}')]
            
            if not manifest_files:
                self.log_to_console(f"✗ No se encontraron manifiestos de soportes en {empaquetado_path}")
                return
            ...
            for i, manifest_file in enumerate(manifest_files, 1):
                try:
                    # Actualizar interfaz
                    status_label.config(text=f"Procesando manifiesto {i}/{total_manifests}...")
                    progress_bar['value'] = (i / total_manifests) * 100
                    counter_label.config(text=f"{i}/{total_manifests}")
                    progress_window.update()
                    
                    # Leer el manifiesto
                    manifest_path = os.path.join(empaquetado_path, manifest_file)
                    invoice_numbers = []
                    ...
                    with open(manifest_path, 'r', encoding='utf-8') as f:
                        ...
                        for line in lines:
                            if line.strip().startswith('- '):
                                invoice_numbers.append(line.strip()[2:])
                    
                    if not invoice_numbers:
                        self.log_to_console(f"✗ No se encontraron números de factura en {manifest_file}")
                        continue
                    
                    # Extraer número de paquete del nombre del manifiesto
                    package_number = manifest_file.split('_')[3]
                    
                    # Crear archivo ZIP para las facturas
                    zip_filename = f"FACTURAS_{regime}_PAQUETE_{package_number}.zip"
                    zip_path = os.path.join(empaquetado_path, zip_filename)
                    
                    # Crear manifiesto para las facturas
                    manifest_filename = f"FACTURAS_{regime}_PAQUETE_{package_number}_MANIFIESTO.txt"
                    manifest_path = os.path.join(empaquetado_path, manifest_filename)
                    
                    with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
                        # Agregar carpetas de facturas al ZIP
                        for invoice_number in invoice_numbers:
                            invoice_folder = os.path.join(facturas_path, invoice_number)
                            if os.path.exists(invoice_folder):
                                # Agregar todos los archivos de la carpeta
                                for root, _, files in os.walk(invoice_folder):
                                    for file in files:
                                        file_path = os.path.join(root, file)
                                        arcname = os.path.join(invoice_number, file)
                                        zipf.write(file_path, arcname)
                            else:
                                self.log_to_console(f"✗ No se encontró la carpeta de factura: {invoice_number}")
                    
                    # Crear archivo de manifiesto
                    with open(manifest_path, 'w', encoding='utf-8') as f:
                        f.write(f"MANIFIESTO DE FACTURAS - {regime}\n")
                        f.write(f"Paquete: {package_number}\n")
                        f.write(f"Fecha: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                        f.write("\nFacturas incluidas:\n")
                        for invoice_number in invoice_numbers:
                            f.write(f"- {invoice_number}\n")
                    
                    self.log_to_console(f"✓ Paquete de facturas {package_number} creado exitosamente")
                    
                except Exception as e:
                    self.log_to_console(f"✗ Error al procesar el manifiesto {manifest_file}: {str(e)}")
            
            # Cerrar ventana de progreso
            progress_window.destroy()
            
            self.log_to_console(f"✓ Proceso de empaquetado de facturas completado")
